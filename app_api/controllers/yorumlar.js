...
const yorumEkle=function(req, res) {
  ...
  if (mekanid){
    ...
    //jsonCevapYolla(res, 404,{"mesaj":"Bulunamadi. mekanid gerekli"});
  ...
  if (req.params && req.params.mekanid && req.params.yorumid){
    ...
    .exec( function(hata, mekan){
        ...
        if (mekan.yorumlar && mekan.yorumlar.length > 0){
          ...
          //varsa cevap nesnesini döndür. içine mekan adı, idsi ve yorumu ekle
          else {
            cevap={
              mekan: {
                ad : mekan.ad,
    ...
    cevapOlustur (res, 404, {"mesaj": "Bulunamadı. mekanid ve yorumid mutlaka girilmeli."});
  }
};

const yorumGuncelle=function(req, res) {
  if(!req.params.mekanid || !req.params.yorumid){
    cevapOlustur(res, 404, {"mesaj":"Bulunamadi. mekanid ve yorumid zorunlu."});
    return;
  }
  Mekan
      ...
      function(hata, gelenMekan){
        if (!gelenMekan){
          ...
          if(gelenMekan.yorumlar && gelenMekan.yorumlar.length > 0){
            ...
            cevapOlustur(res, 404, {"mesaj":"Silinecek yorum bulunamadi"});
          }
      });
};
